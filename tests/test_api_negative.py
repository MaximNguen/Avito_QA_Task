...
class TestCRUDNegative:
    ...
    @allure.title("Создаем новое объявление с невалидным sellerID - негативный тест-кейс №2")
    def test_create_object_without_seller(self, api_factory):
        with allure.step("Создаем объявление с помощью Page Object"):
            new_object = api_factory.create_endpoint()
            new_object.action_wrong(Payloads.create_payload_no_sellerID)

        with allure.step("Проверяем статус кода"):
            new_object.check_status_code(400)

    @allure.title("Создаем новое объявление с отрицательной ценой - негативный тест-кейс №3")
    def test_create_object_with_negative_price(self, api_factory):
        with allure.step("Создаем объявление с помощью Page Object"):
            new_object = api_factory.create_endpoint()
            new_object.action(Payloads.create_payload_with_negative_price)

        with allure.step("Проверяем статус кода"):
            new_object.check_status_code(200)

    @allure.title("Не получаем созданное объявление - негативный тест-кейс №4")
    ...
    @allure.title("Пробуем удалить несуществующий/невалидный тип ID объявление - негативный тест-кейс №7")
